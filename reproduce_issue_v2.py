...
def inspect_optimization(kernel, kernel_name, n_points=20, d=2):
    ...
    dists = pdist(X_opt)
    # distance check on torus
    # (Use optimizer's wrap logic effectively or just check raw uniqueness if they collapsed)
    # Just round and check unique
    unique_points = np.unique(np.round(X_opt, 3), axis=0) # Round to 3 decimals
    print(f"I: Number of unique points (rounded to 3 decimals): {len(unique_points)}")
    if len(unique_points) < n_points:
        print("I: COLLAPSE DETECTED!")
        print("I: Unique points:")
        print(unique_points)
